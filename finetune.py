...
# Code in this file mostly adapted from finetune.py from the PSMNet Github
# Adapted to work with the Argoverse API dataset, and to run without errors
# without a GPU/cuda
...
all_right_img = []
all_left_disp = []
for log_id in train_log_ids[:num_logs]:
...
if loadmodel is not None:
    if cuda:
        ...
        state_dict['state_dict'] = {(k if 'module' not in k else k[7:]): v for k, v in state_dict['state_dict'].items()}
...
def main():
    ...
    for epoch in range(1, epochs+1):
        ...
        print('epoch %d total training loss = %.3f, epoch time = %.2f' %(epoch, total_train_loss/len(TrainImgLoader), time.time() - start_full_time))
